...
<div>
    ...
    if ($request->isPurchaseOrder()) {
        echo T_('Un Bon de commande a été modifié par le créateur suite à votre demande');
    }
    ?>
    <?php
    if ($request->isMissionOrder()) {
        echo T_('Un Ordre de Mission a été modifié par le créateur suite à votre demande');
    }
    ?>.<br>
    <?php
    if ($request->isPurchaseOrder()) {
