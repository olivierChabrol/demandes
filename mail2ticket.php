	...
	if(!$mailsIds) {
		...
		foreach ($tab_MailsInfos as $tab_MailsInfo){
			if($tab_MailsInfo->seen==0)
			{
				...
				/* /!\ AJOUTER par nos soins - permet de ne pas traiter les mails de retour d'absence*/
				$mailhead=$con_mailbox -> getMailHeader($tab_MailsInfo->uid);
				/* Fin ajout*/
				...
				//!\ OLD if($blacklist_mail==1) {
				/*/!\ AJOUTER par nos soins */ if($blacklist_mail==1 || preg_match("/vacation/",$mailhead->headersRaw)) {
